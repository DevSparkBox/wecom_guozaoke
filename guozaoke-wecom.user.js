...
// @connect      imgur.com
// @icon         https://www.guozaoke.com/favicon.ico
// @homepageURL  https://github.com/DevSparkBox/wecom_guozaoke
// @updateURL    https://raw.githubusercontent.com/DevSparkBox/wecom_guozaoke/main/guozaoke-wecom.meta.js
// @downloadURL  https://raw.githubusercontent.com/DevSparkBox/wecom_guozaoke/main/guozaoke-wecom.user.js
// @grant        none
...
(function () {
  ...
  const IS_GZK = true;
  const IS_V2EX_HOST = false;
  const IS_V2EX = true;
  const IS_LINUXDO = false;
  const CURRENT_PLATFORM = "gzk";

  const WATERMARK_ENABLED_KEY = "linuxdo-wecom-watermark-enabled";
  const WATERMARK_TEXT_KEY = "linuxdo-wecom-watermark-text";
  const DEFAULT_WATERMARK_TEXT = IS_GZK ? "guozaoke.com · 内部资料" : (IS_V2EX_HOST ? "v2ex.com · 内部资料" : "linux.do · 内部资料");
  ...
  function getCurrentUserIdentity() {
    ...
    if (IS_GZK) {
      const userLink = document.querySelector(".navbar-right a[href^='/u/'], .navbar a[href^='/u/'], .login-box a[href^='/u/']");
      if (userLink) {
        const match = (userLink.getAttribute("href") || "").match(/\/u\/([^/?#]+)/);
        if (match && match[1]) {
          rememberCurrentUser({ username: decodeURIComponent(match[1]), id: decodeURIComponent(match[1]) });
  ...
  function isTopicPath(pathname) {
    if (IS_GZK && /^\/t\/create\b/.test(pathname)) return false;
  ...
  function isHomePath(pathname) {
    if (IS_GZK) {
      return pathname === "/" || pathname === "/notifications" || /^\/node\//.test(pathname);
    }
    if (IS_V2EX) {
  ...
  function listApiForPath(pathname, search = "") {
    if (IS_GZK) {
      ...
      if (/^\/node\//.test(pathname) || pathname === "/notifications") {
  ...
  const RAW_CSS = String.raw`
    ...
    .wecom-reply-children .wecom-msg-content {
      min-width: 0;
      flex: 1;
    }
  ...
  const SCRIPT_REPOSITORY_URL = "https://github.com/DevSparkBox/wecom_guozaoke";
  const SCRIPT_UPDATE_URL = "https://raw.githubusercontent.com/DevSparkBox/wecom_guozaoke/main/guozaoke-wecom.meta.js";
  const SCRIPT_DOWNLOAD_URL = "https://raw.githubusercontent.com/DevSparkBox/wecom_guozaoke/main/guozaoke-wecom.user.js";
  ...
  function parseUpdateMetadata(text) {
    ...
    if (field("name") !== "过早客 企业微信主题" || field("namespace") !== "https://www.guozaoke.com/" || !validScriptVersion(version)) {
  ...
  function getOrgName() {
    const defaultOrg = IS_GZK ? "guozaoke.com" : (IS_V2EX_HOST ? "v2ex.com" : "linux.do");
    try { return localStorage.getItem(ORG_NAME_KEY) || defaultOrg; } catch { return defaultOrg; }
  }

  function getOrgIcon() {
    const defaultIcon = IS_GZK ? "过" : (IS_V2EX_HOST ? "v2" : "do");
  ...
  function clearNotificationBadge() {
    ...
    if (IS_V2EX) {
      const notifLink = document.querySelector("#Top a[href^='/notifications'], #Rightbar a[href^='/notifications'], .navbar a[href^='/notifications'], a[href='/notifications']");
      if (notifLink) {
  ...
  function syncRail() {
    ...
    if (disguisePreset) {
      ...
      // 原生头像兜底（若选择 native）
      const img = document.querySelector(IS_GZK ? ".navbar img.avatar, .navbar-right img.avatar, .profile img.avatar" : (IS_V2EX ? "#Rightbar .avatar, #Top .avatar, #current-user img" : "#current-user img"));
  ...
  const DEFAULT_GZK_LIST_NAV = [
    { href: "/", label: "默认排序" },
    { href: "/?tab=latest", label: "最新话题" },
    { href: "/?tab=elite", label: "精华主题" },
    { href: "/?tab=interest", label: "兴趣节点" },
    { href: "/?tab=follows", label: "关注的人" }
  ];

  const DEFAULT_GZK_NODES = [
    { href: "/node/water", label: "汤逊湖" },
    { href: "/node/qna", label: "你问我答" },
    { href: "/node/job", label: "找工作" },
    { href: "/node/IT", label: "IT技术" },
    { href: "/node/city", label: "城市建设" },
    { href: "/node/finance", label: "金融财经" },
    { href: "/node/house", label: "楼市房产" },
    { href: "/node/lowshine", label: "同城活动" },
    { href: "/node/startup", label: "创业创客" },
    { href: "/node/date", label: "非诚勿扰" },
    { href: "/node/2ndhand", label: "二手交易" },
    { href: "/node/auto", label: "汽车" },
    { href: "/node/digital", label: "硬件数码" },
    { href: "/node/food", label: "美食" },
    { href: "/node/dandelion", label: "狮子的牙齿" }
  ];
  ...
  function extractGzkNodesFromDom(root = document) {
    const cloud = root.querySelector(".nodes-cloud, .nodes");
    const links = [...(cloud ? cloud.querySelectorAll("a[href^='/node/']") : root.querySelectorAll(".hotlink a[href^='/node/'], a[href^='/node/']"))]
      .map((a) => ({
        href: a.getAttribute("href") || "#",
        label: (a.textContent || "").replace(/\s+/g, " ").trim(),
        active: false
      }))
      .filter((it) => it.label && it.href && it.href.startsWith("/node/"));
    const seen = new Set();
    const deduped = [];
    for (const it of links) {
      if (seen.has(it.href)) continue;
      seen.add(it.href);
      it.active = typeof location !== "undefined" && location.pathname === it.href.split("?")[0];
      deduped.push(it);
    }
    return deduped.length ? deduped : null;
  }

  function getGzkNav2Items() {
    const fromDom = extractGzkNodesFromDom(document);
    if (fromDom && fromDom.length) return fromDom;
    const path = typeof location !== "undefined" ? location.pathname : "/";
    return DEFAULT_GZK_NODES.map((it) => ({ ...it, active: path === it.href }));
  }

  function extractV2exTabsFromDom(root = document) {
    if (IS_GZK) {
      const pills = root.querySelectorAll(".nav.nav-pills a");
      if (pills.length) {
        const links = [...pills].map((a) => ({
          href: a.getAttribute("href") || "/",
          label: (a.textContent || "").replace(/\s+/g, " ").trim(),
          active: a.parentElement?.classList.contains("active") || a.classList.contains("active")
        })).filter((it) => it.label && it.href && !it.href.startsWith("javascript:"));
  ...
  function getV2exTabs() {
    ...
    return IS_GZK ? DEFAULT_GZK_LIST_NAV : DEFAULT_V2EX_LIST_NAV;
  }
  ...
  function syncV2exNav2() {
    ...
    if (!listEl) return;
    const items = IS_GZK ? getGzkNav2Items() : collectListNavItems();
  ...
  async function syncV2exUserStats(forceNetwork = false) {
    if (!IS_V2EX_HOST || isFetchingV2exStats) return null;
  ...
  async function checkinV2exDaily(manual = false) {
    if (!IS_V2EX_HOST || isCheckingV2exDaily) return;
    const now = new Date();
  ...
  function updateOpenV2exUserPopover(stats) {
    ...
    if (stats.username) {
      ...
      if (userLink) userLink.href = IS_GZK ? `/u/${encodeURIComponent(stats.username)}` : `/member/${encodeURIComponent(stats.username)}`;
    }
  ...
  function openV2exUserPopover() {
    ...
    const profileUrl = username ? (IS_GZK ? `/u/${encodeURIComponent(username)}` : `/member/${encodeURIComponent(username)}`) : "javascript:void(0)";
  ...
  function parseGzkMemberProfile(html, username) {
    if (!html || typeof html !== "string") return null;
    const doc = new DOMParser().parseFromString(html, "text/html");
    const profileRoot = doc.querySelector(".profile, .user-page .profile");
    if (!profileRoot && !doc.querySelector(".user-page")) return null;
    const profile = {
      username: (profileRoot?.querySelector(".username")?.textContent || username || "").trim(),
      avatarUrl: "",
      uid: "",
      isOnline: false,
      tagline: "",
    ...
    const avatar = (profileRoot || doc).querySelector("img.avatar");
    if (avatar?.getAttribute("src")) {
      let src = avatar.getAttribute("src");
      if (src.startsWith("//")) src = "https:" + src;
      profile.avatarUrl = src;
    }
    const numberText = (profileRoot?.querySelector(".user-number .number, .number")?.textContent || "").replace(/\s+/g, " ");
    const numMatch = numberText.match(/(\d+)/);
    if (numMatch) {
      profile.uid = numMatch[1];
      profile.memberNum = numMatch[1];
    }
    const sinceText = (profileRoot?.querySelector(".since")?.textContent || "").trim();
    const joinedMatch = sinceText.match(/(\d{4}-\d{2}-\d{2})/);
    if (joinedMatch) profile.joinedDate = joinedMatch[1];
    const dls = [...(profileRoot?.querySelectorAll("dl") || [])];
    for (const dl of dls) {
      const dt = (dl.querySelector("dt")?.textContent || "").trim();
      const dd = (dl.querySelector("dd")?.textContent || "").trim();
      if (dt === "签名" || dt === "昵称") {
        if (!profile.tagline) profile.tagline = dd;
        else if (dt === "签名") profile.intro = dd;
      }
    }
    const followA = doc.querySelector("a[href^='/f/user/'], a[href^='/uf/user/']");
    if (followA) {
      profile.followUrl = followA.getAttribute("href") || "";
      profile.isFollowed = /取消关注|已关注|unfollow|\/uf\//i.test(followA.textContent + profile.followUrl);
    } else {
      profile.followUrl = "";
    ...
    const topicItems = doc.querySelectorAll(".topic-item h3.title a, .topic-item .title a[href*='/t/']");
    topicItems.forEach((a) => {
      if (profile.recentTopics.length >= 2) return;
  ...
  function parseV2exMemberProfile(html, username) {
    if (!html || typeof html !== "string") return null;
    if (IS_GZK || (/class=["']profile/.test(html) && /入住于|过早客第/.test(html))) {
      const gzkProfile = parseGzkMemberProfile(html, username);
      if (gzkProfile) return gzkProfile;
    }
  ...
  async function fetchV2exMemberProfile(username) {
    ...
    try {
      const memberPath = IS_GZK ? `/u/${encodeURIComponent(username)}` : `/member/${encodeURIComponent(username)}`;
      const resp = await fetch(memberPath, { credentials: "include" });
      if (resp.status === 404) {
        const notFoundData = {
          username,
          isNotFound: true,
  ...
  function openV2exMemberCard(username, triggerEl, event) {
    ...
    card.innerHTML = `
      ...
      <div class="wecom-member-card-footer">
        <a class="wecom-member-btn primary" href="${IS_GZK ? `/u/${encodeURIComponent(username)}` : `/member/${encodeURIComponent(username)}`}" target="_blank">查看完整主页 ↗</a>
    ...
    if (IS_V2EX) {
      ...
      return tabs.map((it) => {
        ...
        if (hasQuery) {
          ...
          active = hasDomActive ? Boolean(it.active) : (IS_GZK ? it.href === "/" : (it.href === "/?tab=all" || it.href === "/?tab=tech"));
  ...
  function submitNativeSearch(value) {
    ...
    if (!q) return;
    if (IS_GZK) {
      window.open(`https://cn.bing.com/search?q=site:guozaoke.com/t%20${encodeURIComponent(q)}`, "_blank");
      return;
    }
    if (IS_V2EX) {
  ...
  function openNewTopic() {
    if (IS_GZK) {
      const nodeMatch = String(location.pathname || "").match(/^\/node\/([^/?#]+)/);
      const node = nodeMatch ? nodeMatch[1] : "qna";
      window.open(`/t/create/${encodeURIComponent(node)}`, "_blank", "noopener,noreferrer");
      return;
    }
    if (IS_V2EX) {
  ...
  function ensureListPanel() {
    ...
    const searchPlaceholder = IS_GZK ? "搜索过早客话题" : (IS_V2EX ? "搜索 V2EX 话题" : "搜索");
    const chipsHtml = IS_GZK
      ? `
          <button type="button" class="wecom-chip active" data-chip="all">消息<span class="n"></span></button>
          <button type="button" class="wecom-chip" data-chip="latest">最新</button>
          <button type="button" class="wecom-chip" data-chip="elite">精华</button>
          <button type="button" class="wecom-chip" data-chip="interest">兴趣</button>
        `
      : (IS_V2EX
      ? `
          <button type="button" class="wecom-chip active" data-chip="all">消息<span class="n"></span></button>
    ...
    panel.innerHTML = `
      <div class="wecom-list-search">
        <form action="${IS_GZK ? "https://cn.bing.com/search" : (IS_V2EX ? "https://www.google.com/search" : "/search")}" method="get"${IS_V2EX ? ' target="_blank"' : ""} role="search">
          ${ICONS.search}
    ...
    panel.querySelectorAll(".wecom-chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        ...
        if (IS_GZK) {
          ...
          const gzkChipMap = { all: "/", latest: "/?tab=latest", elite: "/?tab=elite", interest: "/?tab=interest", follows: "/?tab=follows" };
          const targetPath = gzkChipMap[chipKey] || "/";
          if (location.pathname + location.search !== targetPath) {
            navigateInApp(targetPath);
          } else {
            loadList(targetPath, true);
          }
        } else if (IS_V2EX) {
          const chipKey = chip.dataset.chip;
  ...
  function convCategoryTag(topic) {
    ...
    if (!isExt) return "";
    return `<span class="wecom-conv-tag is-ext">@${escapeHtml(name)}</span>`;
  }
  ...
  function switchToListMode(mode) {
    ...
    if (panel) {
      ...
      if (searchInput) {
        searchInput.value = "";
        searchInput.placeholder = mode === "history" ? "搜索历史帖子" : (IS_GZK ? "搜索过早客话题" : (IS_V2EX ? "搜索 V2EX 话题" : "搜索"));
      }
    }
    if (mode === "history") {
      renderHistoryList();
  ...
  function parseV2exListPagination(doc, apiPath, count) {
    if (IS_GZK) return parseGzkListPagination(doc, apiPath, count);
    const path = String(apiPath || "");
    const currentPage = v2exPageForPath(path);
  ...
  async function loadList(apiPath, force) {
    ...
    if (IS_V2EX) {
      ...
      try {
        ...
        if (canUseDom && document.querySelectorAll("#Main .cell, #Main .item, .topic-item").length > 0) {
        ...
        <div class="wecom-chat-tools">
          ...
          <div class="wecom-platform-switcher">
            ...
            <div class="wecom-platform-dropdown" hidden>
              ...
              <div class="wecom-platform-item${IS_V2EX_HOST ? " is-active" : ""}" data-target-platform="v2ex">
                ...
                ${IS_V2EX_HOST ? '<span class="wecom-platform-item-check">✓</span>' : ""}
              </div>
              <div class="wecom-platform-item${IS_GZK ? " is-active" : ""}" data-target-platform="gzk">
                <span class="wecom-platform-item-icon">🌅</span>
                <div class="wecom-platform-item-info">
                  <div class="wecom-platform-item-name">过早客</div>
                  <div class="wecom-platform-item-desc">guozaoke.com · 武汉本地社区</div>
                </div>
                ${IS_GZK ? '<span class="wecom-platform-item-check">✓</span>' : ""}
              </div>
            </div>
          </div>
  ...
  function handleChatHeaderClick(event, panel) {
    ...
    if (event.target.closest(".wecom-platform-item")) {
      ...
      } else if (target === "v2ex" && IS_GZK) {
        window.location.href = "https://www.v2ex.com/";
      } else if (target === "gzk" && !IS_GZK) {
        window.location.href = "https://www.guozaoke.com/";
      }
      return true;
    }
  ...
  function extractReferencedReply(content) {
    ...
    const memberMatch = unwrapped.match(/^(?:@\s*)?<a\s+[^>]*href=["']\/(?:member|u)\/([^"'/?#]+)["'][^>]*>(?:@)?([^<]*)<\/a>/i);
    if (memberMatch) {
  ...
  async function toggleLike(postId, btn, msgEl) {
    ...
    try {
      if (IS_V2EX) {
        ...
        if (wasLiked) {
          showWecomToast(IS_GZK ? "已经赞过了" : "已经感谢过该回复了", "info", 1800);
          return;
        }
        if (IS_GZK) {
          const isTopic = Number(msg?.dataset?.postNumber) === 1 || Number(msg?.dataset?.floor) === 0;
          const targetLabel = isTopic ? "赞主题" : "赞回复";
          ...
          showWecomToast(isTopic ? "已赞主题" : "已赞回复", "success", 1800);
          try {
            const voteUrl = isTopic
              ? `/vote?topic_id=${encodeURIComponent(chatState.topicId || postId)}`
              : `/replyVote?reply_id=${encodeURIComponent(postId)}`;
            const resp = await fetch(voteUrl, { credentials: "same-origin" });
            const data = await resp.json().catch(() => null);
            if (!resp.ok || data?.success === false) {
              const errMap = {
                topic_not_exist: "主题不存在",
                reply_not_exist: "回复不存在",
                can_not_vote_your_topic: "不能赞自己的主题",
                can_not_vote_your_reply: "不能赞自己的回复",
                already_voted: "已经赞过了",
                user_not_login: "请先登录过早客"
              };
              throw new Error(errMap[data?.message] || data?.message || `HTTP ${resp.status}`);
            }
          } catch (err) {
  ...
  function absolutizeSiteUrl(src) {
  ...
  function extractGzkTopicsFromDoc(doc) {
    if (!doc) return [];
    const items = doc.querySelectorAll(".topic-item");
    const topics = [];
    items.forEach((item) => {
      const titleLink = item.querySelector("h3.title a, .title a[href*='/t/']");
      if (!titleLink) return;
      ...
      const target = parseV2exReplyTarget(href);
      const replies = parseInt((item.querySelector(".count a, .count")?.textContent || "").replace(/\D/g, ""), 10) || 0;
      const author = (item.querySelector(".username a")?.textContent || "").trim();
      const lastReply = (item.querySelector(".last-reply-username strong, .last-reply-username a")?.textContent || "").trim();
      topics.push({
        id,
        title: (titleLink.textContent || "").replace(/\s+/g, " ").trim() || `话题 #${id}`,
        posts_count: replies + 1,
        reply_count: replies,
        created_at: (item.querySelector(".last-touched")?.textContent || "").trim(),
        bumped_at: (item.querySelector(".last-touched")?.textContent || "").trim(),
        last_poster_username: lastReply || author,
        v2ex_author: author,
        node_name: (item.querySelector(".node a")?.textContent || "").trim(),
        v2ex_avatar: absolutizeSiteUrl(item.querySelector("img.avatar")?.getAttribute("src") || ""),
  ...
  function parseGzkListPagination(doc, apiPath, count) {
    const path = String(apiPath || "/");
    const currentPage = v2exPageForPath(path);
    if (count === 0) return { hasMore: false, nextPageUrl: null, totalPages: 1 };
    if (doc) {
      const pag = doc.querySelector("ul.pagination");
      if (pag) {
        const nextLi = [...pag.querySelectorAll("li")].find((li) => (li.textContent || "").includes("下一页"));
        const nextA = nextLi?.querySelector("a");
        const href = (nextA?.getAttribute("href") || "").trim();
        const disabled = Boolean(nextLi?.classList.contains("disabled"));
        const hasMore = !disabled && href && href !== "javascript:;";
        const pageNumbers = [...pag.querySelectorAll("a[href*='p=']")]
          .map((a) => {
            const match = (a.getAttribute("href") || "").match(/[?&]p=(\d+)/);
            return match ? Number(match[1]) : 0;
          })
          .filter(Boolean);
        return {
          hasMore,
          nextPageUrl: hasMore ? href : null,
          totalPages: pageNumbers.length ? Math.max(...pageNumbers, currentPage) : (hasMore ? currentPage + 1 : currentPage)
        };
      }
    }
    const nextBase = path === "/?tab=all" || path === "all" || path === "latest" ? "/" : path;
    return {
      hasMore: count >= 20,
      nextPageUrl: v2exPageUrl(nextBase, currentPage + 1),
      totalPages: 0
    };
  }

  function parseGzkTopicDoc(topicId, doc, page = 1) {
    if (!doc) return null;
    const detail = doc.querySelector(".topic-detail");
    if (!detail) return null;
    const title = (detail.querySelector("h3.title, .title")?.textContent || `主题 #${topicId}`).replace(/\s+/g, " ").trim();
    const opUsername = (detail.querySelector(".username a")?.textContent || "").trim() || "gzk_user";
    const opAvatar = absolutizeSiteUrl(detail.querySelector(".ui-header img.avatar")?.getAttribute("src") || "");
    const opContent = detail.querySelector(".ui-content")?.innerHTML?.trim() || "<p>（无正文）</p>";
    const opCreated = (detail.querySelector(".created-time")?.textContent || "").trim();
    const opNode = (detail.querySelector(".node a")?.textContent || "").trim();
    const hitsText = (detail.querySelector(".hits")?.textContent || "").replace(/\s+/g, " ");
    const favText = (detail.querySelector(".favorited")?.textContent || "").replace(/\s+/g, " ");
    const voteText = (detail.querySelector(".up_vote")?.textContent || "").replace(/\s+/g, " ");
    const views = parseInt((hitsText.match(/(\d+)/) || [])[1], 10) || 0;
    const favorites = parseInt((favText.match(/(\d+)/) || [])[1], 10) || 0;
    const thanks = parseInt((voteText.match(/(\d+)/) || [])[1], 10) || 0;
    const favBtn = doc.querySelector(".J_topicFavorite");
    const v2exFavorited = (favBtn?.getAttribute("data-type") || "") === "unfavorite" || (favBtn?.textContent || "").includes("取消收藏");
    const v2exOnce = doc.querySelector("input[name='_xsrf']")?.value?.trim() || "";
    const opPost = {
      id: Number(topicId),
      post_number: 1,
      floor: 0,
      ...
      like_count: thanks,
      actions_summary: thanks > 0 ? [{ id: 2, count: thanks }] : [],
      v2ex_topic_stats: { views, favorites, thanks }
    };
    const posts = [opPost];
    const replyCells = doc.querySelectorAll(".reply-item");
    replyCells.forEach((cell, idx) => {
      const voteA = cell.querySelector("a.J_replyVote");
      const rid = Number(((voteA?.getAttribute("href") || "").match(/reply_id=(\d+)/) || [])[1]) || (idx + 2);
      const username = (cell.querySelector(".reply-username .username, .reply-username, .username a")?.textContent || "").trim() || `用户_${idx + 2}`;
      const avatar = absolutizeSiteUrl(cell.querySelector("img.avatar")?.getAttribute("src") || "");
      const content = cell.querySelector(".content")?.innerHTML || "";
      const timeText = (cell.querySelector(".meta .time")?.textContent || "").trim();
      const likesCount = Number(voteA?.getAttribute("data-count")) || parseInt((voteA?.textContent || "").replace(/\D/g, ""), 10) || 0;
      const floorEl = cell.querySelector(".meta > span.fr.floor, span.fr.floor:not(.reply-to)");
      const floorNo = floorEl ? parseInt((floorEl.textContent || "").replace(/\D/g, ""), 10) : ((page - 1) * 100 + idx + 1);
      posts.push({
        id: rid,
        floor: floorNo,
    ...
    let totalReplies = Math.max(0, posts.length - 1);
    const countText = (doc.querySelector(".topic-reply .ui-header")?.textContent || "").replace(/\s+/g, " ");
    const countMatch = countText.match(/(\d+)\s*条回复/) || (doc.body?.textContent || "").match(/共收到\s*(\d+)\s*条回复/);
    if (countMatch) totalReplies = Number(countMatch[1]) || totalReplies;
    const pag = doc.querySelector(".topic-reply ul.pagination, ul.pagination");
    const pageNumbers = pag ? [...pag.querySelectorAll("a[href*='p=']")].map((a) => {
      const match = (a.getAttribute("href") || "").match(/[?&]p=(\d+)/);
      return match ? Number(match[1]) : 0;
    }).filter(Boolean) : [];
    const nextLi = pag ? [...pag.querySelectorAll("li")].find((li) => (li.textContent || "").includes("下一页")) : null;
    const hasNextBtn = Boolean(nextLi && !nextLi.classList.contains("disabled"));
    const totalPages = Math.max(pageNumbers.length ? Math.max(...pageNumbers) : 0, hasNextBtn ? page + 1 : page, page);
    resolveV2exReplyRelationships(posts);
    return {
      id: Number(topicId),
      title,
      ...
      total_pages: totalPages || (hasNextBtn ? page + 1 : page),
      node_name: opNode,
      node_title: opNode,
      ...
      v2ex_page: page,
      v2ex_has_more: hasNextBtn || totalPages > page
    };
  }

  function extractV2exTopicsFromDoc(doc) {
    if (!doc) return [];
    const gzkTopics = extractGzkTopicsFromDoc(doc);
    if (gzkTopics.length) return gzkTopics;
  ...
  function parseV2exTopicDoc(topicId, doc, page = 1) {
    if (!doc) return null;
    if (doc.querySelector(".topic-detail")) {
      const parsed = parseGzkTopicDoc(topicId, doc, page);
  ...
  async function fetchV2exTopicHtml(topicId, signal, page = 1) {
    ...
    if (resp.redirected && (resp.url.includes("/signin") || resp.url.includes("/login"))) {
      throw new Error(IS_GZK ? "查看该主题需要先登录过早客" : "查看该主题需要先登录 V2EX");
    }
    const html = await resp.text();
    const doc = new DOMParser().parseFromString(html, "text/html");
  ...
  async function toggleV2exTopicFavorite() {
    const topicId = Number(chatState.topicId);
    if (!topicId) throw new Error(IS_GZK ? "未能确定当前过早客话题" : "未能确定当前 V2EX 话题");
    const isFavorited = Boolean(chatState.v2exTopicFavorited);
    if (IS_GZK) {
      const url = isFavorited ? `/unfavorite?topic_id=${encodeURIComponent(topicId)}` : `/favorite?topic_id=${encodeURIComponent(topicId)}`;
      ...
      setV2exTopicFavoriteState(!isFavorited);
      updateV2exTopicFavoriteStats(!isFavorited);
      showWecomToast(isFavorited ? "已取消收藏" : "已收藏话题", "success", 1800);
      return;
    }
  ...
  async function fetchTopicJson(topicId, postNumber, force, signal, targetPage = 1) {
    ...
    if (IS_V2EX) {
      ...
      if (!force && page === 1 && id === INITIAL_V2EX_TOPIC_ID && !initialV2exTopicConsumed &&
          document.querySelector("#Main .topic_content, .topic-detail .ui-content") &&
  ...
  async function loadTopic(topicId, force = false, targetOption = null) {
    ...
    try {
      ...
      const orgName = IS_GZK ? (data.node_title || data.node_name || "guozaoke.com") : (IS_V2EX ? (data.node_title || data.node_name || "v2ex.com") : "linux.do");
      if (sub) {
  ...
  async function pollV2exCurrentTopicOnce() {
    ...
    try {
      ...
      const doc = new DOMParser().parseFromString(html, "text/html");
      const remoteNotif = doc.querySelector("#Top a[href^='/notifications'], #Rightbar a[href^='/notifications'], .navbar a[href^='/notifications'], a[href='/notifications']");
      if (remoteNotif) {
  ...
  function applyV2exRemoteNotifCount(count) {
    ...
    const localLinks = document.querySelectorAll("#Top a[href^='/notifications'], #Rightbar a[href^='/notifications'], .navbar a[href^='/notifications'], a[href='/notifications']");
  ...
  async function pollV2exNotificationsOnce() {
    ...
    try {
      ...
      const html = await resp.text();
      const doc = new DOMParser().parseFromString(html, "text/html");
      const notifLink = doc.querySelector("#Top a[href^='/notifications'], #Rightbar a[href^='/notifications'], .navbar a[href^='/notifications'], a[href='/notifications']");
      if (notifLink) {
        const text = notifLink.textContent || "";
        const m = text.match(/\d+/);
